  ...
  try {
    ...
    if (
      ...
      res.status(400).json({
        success: false,
        message:
          "Faltan campos obligatorios: id, name, type, description, genre, franchise, number, exclusive, specialFeatures o marketValue",
      });
    }
...
app.delete("/funkos/:usuario/:id", (req: Request, res: Response) => {
  const usuario = req.params.usuario as string;
...
app.patch("/funkos/:usuario/", (req: Request, res: Response) => {
  try {
    ...
    const response: ResponseType = executeCommand(usuario, "update", [
      id.toString(),
      name,
      description,
      type,
    ...
    if (response.success) {
      res.json(response); // 201 Created si es exitoso
    } else {
      res.json(response); // 409 Conflict si hay error (ej: ID duplicado)
    }
  } catch (error) {
    res.status(500).json({
      success: false,
